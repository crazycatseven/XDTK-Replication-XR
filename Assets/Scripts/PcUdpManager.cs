...
//         rayLineRenderer.SetPosition(0, phone.transform.position + phone.transform.up * 0.12f);
...
//                     // 获得 "Position:" 后的数据
//                     string positionDataHit = payload.Substring(payload.IndexOf("Position:") + "Position:".Length);
//                     // 去除空格
//                     positionDataHit = positionDataHit.Trim();
//                     // 解析位置数据
//                     string[] positionDataArrayHit = positionDataHit.Split(',');
//                     float px = float.Parse(positionDataArrayHit[0]);
//                     float py = float.Parse(positionDataArrayHit[1]);
//                     float pz = float.Parse(positionDataArrayHit[2]);

//                     Vector3 relativePosition = new Vector3(px, py, pz);
//                     Vector3 spawnPosition = phoneParent.transform.TransformPoint(relativePosition);
...
//                             // phoneParent.transform.position = positionOffset;
...
//     public void boxAlign(){

//         // 当按下后，AR相机的Origin为当前的位置，对应当前场景中手机摄像头的位置

//         Vector3 virtualPhoneCameraPosition;
...
//         // 获得手机模型位置和旋转（世界坐标）
//         Vector3 phoneModelPosition = phoneModel.transform.position;
    
//         Quaternion phoneModelRotation = phoneModel.transform.rotation;

//         positionOffset = phoneModelPosition;
//         rotationOffset = phoneModelRotation * Quaternion.Euler(0, 180, 0);
//     }
